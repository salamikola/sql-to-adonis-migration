...
import {ForeignConstraintType, TableIndexType, TableInfoType, TableSchemaType} from "../../types";
...
export default class MysqlQueryManger implements QuerySchemaContract {
  ...
  async getTableConstraints(table: string): Promise<ForeignConstraintType[]> {
    const constraints =  await this.client.rawQuery(`SELECT
...
WHERE
  ...
  async getTableSchema(table: string): Promise<TableSchemaType[]> {
    const tableSchema = await this.client.rawQuery(`SELECT COLUMN_NAME,COLUMN_DEFAULT,IS_NULLABLE,DATA_TYPE,CHARACTER_MAXIMUM_LENGTH,CHARACTER_SET_NAME,COLLATION_NAME,COLUMN_TYPE,COLUMN_KEY,EXTRA,COLUMN_COMMENT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?`,
      [this.dbName, table])
